...
def discriminator(x, x_batch, x_size, model, layers, hidden_dim, leaky_rate=0.01, condition=None):
	'''
	Define the achitecture of the discriminator

	Inputs:
		- x: TensorFlow tensor of flattened input images, shape [batch_size, img_dim * img_dim]
	...
	if model == 'dnn':
		...
		with tf.variable_scope('discriminator', reuse=tf.AUTO_REUSE):
			...
			for l in range(layers):
				...
				x = tf.nn.leaky_relu(x, alpha=leaky_rate)
...
def generator(x, x_size, model, layers, hidden_dim):
	'''
	...
	if model == 'dnn':
		with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
			...
			products = tf.layers.dense(inputs = x, units = x_size * x_size, use_bias=True, activation=tf.tanh, name='products')
			return products
	elif model == 'cnn':
		with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
			...
			x = tf.layers.dense(inputs = x, units = x_size//4 * x_size//4 * hidden_dim[-1], activation=tf.nn.relu)
			x = tf.layers.batch_normalization(x, training=True)
			x = tf.reshape(x, (-1, x_size//4, x_size//4, hidden_dim[-1]))
			# The de-conv layer
...
def get_mnist_condition(batch_size, labels):
	'''
	return the condition image (as [batch_size * 28 * 28 * 10]) given the number

	Inputs:
		- labels: numpy array with shape of batch_size * 1
	Outputs:
		- condition with shape of batch_size * 28 * 28 * 10
	'''
